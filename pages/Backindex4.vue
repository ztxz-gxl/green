          ...
          <div class="card">
            <div class="card-body">
              <div class="form-group">
                <p style="font-size: 2.5rem">取出绿色银行代币</p>
                <input
                  type="text"
                  class="form-control"
              ...
              <div class="form-group">
                <p style="font-size: 2.5rem">存入绿色银行代币</p>
                <input
                  type="text"
                  class="form-control"
                  style="margin-bottom: 1%"
...
export default {
  ...
  asyncData() {
    return axios.get(`/api/rep`).then((res) => {
      return {
        list: res.data.list,
      };
    });
  ...
  methods: {
    CRBank() {
      let num = document.getElementById("CRNum").value;
      return axios
        .get(`/api/CRBank?id=` + this.id + "&num=" + num)
        .then((res) => {
          if (res.status == 200) {
    ...
    QCBank() {
      let num = document.getElementById("QCNum").value;
      return axios
        .get(`/api/QCBank?id=` + this.id + "&num=" + num)
        .then((res) => {
          if (res.status == 200) {
            alert(res.data.massage);
          }
